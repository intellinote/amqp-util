#!/bin/bash
# This script will run a JavaScript/CoffeeScript file of the same name found in `../lib/app/`.
BASENAME="$(basename "$0")"
if [ "$BASENAME" = ".shebang.sh" ]; then
  ...
  if [ -x "$COFFEE_EXE" ]; then
    "$COFFEE_EXE" "$BASEDIR/lib/app/$BASENAME.coffee" "$@"
  else
    "$NODE_EXE" "$BASEDIR/lib/app/$BASENAME.js" "$@"
  fi
fi
